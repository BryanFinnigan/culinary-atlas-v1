    ...
    return (
      <main className="min-h-screen bg-orange-50 px-6 py-10 text-slate-950 sm:px-10 lg:px-16">
        <div className="mx-auto max-w-3xl rounded-[2rem] bg-white p-8 shadow-sm">
          <h1 className="text-4xl font-black">Cuisine map is being prepared</h1>
          <p className="mt-4 text-slate-700">Restore cuisine data to populate the map experience.</p>
        </div>
      </main>
    );
  }
  ...
  return (
    <main className="min-h-screen bg-orange-50 px-6 py-10 text-slate-950 sm:px-10 lg:px-16">
      <div className="mx-auto max-w-7xl">
        ...
        <h1 className="mt-3 max-w-4xl text-5xl font-black leading-tight tracking-[-0.04em] sm:text-6xl">Explore the world through food.</h1>
        <p className="mt-5 max-w-3xl text-lg leading-8 text-slate-700">Select a country on the map, then continue to its cuisine guide, region page, or taste collections where available.</p>

        <div className="mt-10 grid gap-6 lg:grid-cols-[1.2fr_0.8fr]">
          <WorldMap countries={allCuisines.map((cuisine) => cuisine.name)} selectedCountry={selectedCountry} setSelectedCountry={setSelectedCountry} />
          <aside className="rounded-[2rem] border border-orange-100 bg-white p-6 shadow-sm">
            ...
            <Link href={`/cuisines/${selectedCuisine.slug}`} className="mt-6 inline-flex rounded-full bg-orange-600 px-5 py-3 font-bold text-white hover:bg-orange-700">Open guide</Link>
          </aside>
        </div>
      </div>
    </main>
