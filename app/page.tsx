    ...
    <main className="min-h-screen bg-orange-50 text-slate-950">
      <section className="relative overflow-hidden px-6 py-8 sm:px-10 lg:px-16">
        ...
        <nav className="mx-auto flex max-w-7xl items-center justify-between gap-4 rounded-full border border-orange-200 bg-white/80 px-5 py-3 shadow-sm backdrop-blur">
          ...
          <div className="hidden items-center gap-6 text-sm font-semibold text-slate-600 sm:flex">
            ...
            <a className="transition hover:text-slate-950" href="#products">Collections</a>
            <a className="transition hover:text-slate-950" href="#system">Growth System</a>
          </div>
        </nav>
        ...
        <div id="top" className="mx-auto grid max-w-7xl items-center gap-12 py-20 lg:grid-cols-[1.05fr_0.95fr] lg:py-28">
          <div>
            <p className="mb-5 inline-flex rounded-full bg-slate-950 px-4 py-2 text-sm font-bold uppercase tracking-[0.2em] text-orange-100">
              Global food discovery + affiliate-ready product data
            </p>
            <h1 className="max-w-4xl text-5xl font-black leading-[0.95] tracking-[-0.05em] sm:text-6xl lg:text-7xl">
              Explore cuisines. Build taste collections. Turn data into revenue.
            </h1>
            <p className="mt-6 max-w-2xl text-lg leading-8 text-slate-700 sm:text-xl">
              Culinary Atlas connects cuisine context, regional food culture, and practical products people can try at home. The map is the interface; the structured catalog is the asset.
            </p>
            <div className="mt-8 flex flex-col gap-3 sm:flex-row">
              ...
              <a className="rounded-full border border-orange-200 bg-white px-6 py-3 text-center font-bold text-slate-900 transition hover:-translate-y-0.5 hover:border-orange-400" href="#products">
                View collections
              </a>
            </div>
          </div>
          ...
          <aside className="rounded-[2rem] border border-orange-200 bg-white p-5 shadow-2xl shadow-orange-950/10">
            <div className="rounded-[1.5rem] bg-slate-950 p-6 text-white">
              ...
              <p className="mt-3 leading-7 text-orange-100">
                UK and Italy product catalogs are preserved in JSON and ready to power country pages, collection sections, and future product landing pages.
              </p>
      ...
      <section id="explore" className="bg-slate-50 px-6 py-16 sm:px-10 lg:px-16">
        <div className="mx-auto max-w-7xl">
          <div className="mb-10 grid gap-8 lg:grid-cols-[0.85fr_1.15fr] lg:items-end">
            <div>
              <p className="font-bold uppercase tracking-[0.2em] text-orange-700">Interactive Explorer</p>
              <h2 className="mt-3 text-4xl font-black tracking-tight sm:text-5xl">Explore cuisines by country</h2>
            </div>
            <p className="text-lg leading-8 text-slate-700">
              Search, filter, and select a country to connect the map experience with cuisine summaries, Food & Culture Notes, and collected product data.
            </p>
          </div>
            ...
            <div className="mt-4 flex max-h-44 flex-wrap gap-2 overflow-auto pr-1">
              {visibleCountries.map((country) => (
                <button
                  ...
                  className={`rounded-full px-4 py-2 text-sm font-bold transition ${selectedCountry === country ? "bg-orange-600 text-white shadow" : "bg-white text-slate-700 ring-1 ring-slate-200 hover:bg-orange-50"}`}
                >
                  {country}
                </button>
              ))}
      ...
      <section id="products" className="bg-white px-6 py-16 sm:px-10 lg:px-16">
        <div className="mx-auto max-w-7xl">
          <div className="flex flex-col justify-between gap-4 md:flex-row md:items-end">
            <div>
              <p className="font-bold uppercase tracking-[0.2em] text-orange-700">Curated commerce layer</p>
              <h2 className="mt-3 text-4xl font-black tracking-tight sm:text-5xl">{displayCountry} taste collections</h2>
              <p className="mt-4 max-w-3xl text-lg leading-8 text-slate-700">
                Collections organize affiliate products by experience, not by random links. This supports trust, SEO, and user clarity.
              </p>
            </div>
            <Link className="rounded-full border border-orange-200 px-5 py-3 text-center font-bold text-slate-900 transition hover:border-orange-500 hover:bg-orange-50" href={`/countries/${slugify(displayCountry as string)}`}>
              Open country page
            </Link>
          </div>
          ...
          {selectedProducts.length > 0 ? (
            ...
            <div className="mt-10 rounded-[2rem] border border-dashed border-orange-200 bg-orange-50 p-8 text-slate-700">
              <h3 className="text-2xl font-black text-slate-950">Product catalog coming soon for {displayCountry}</h3>
              <p className="mt-3 max-w-3xl leading-7">Cuisine data is ready. The next monetization step is collecting country-specific products, preserving affiliate links, and building SEO pages around high-intent foods.</p>
            </div>
          )}
        </div>
      </section>
      ...
      <section id="system" className="bg-slate-950 px-6 py-16 text-white sm:px-10 lg:px-16">
        <div className="mx-auto grid max-w-7xl gap-10 lg:grid-cols-[0.8fr_1.2fr]">
          <div>
            <p className="font-bold uppercase tracking-[0.2em] text-amber-300">Six Sigma operating rhythm</p>
            <h2 className="mt-3 text-4xl font-black tracking-tight sm:text-5xl">Define. Measure. Analyze. Improve. Control.</h2>
            <p className="mt-4 leading-7 text-orange-100">The product should improve through repeatable data collection, quality checks, user behavior, and controlled releases.</p>
          </div>
          <ol className="grid gap-4">
            {operatingSteps.map((step, index) => (
              <li key={step} className="flex gap-4 rounded-3xl bg-white/10 p-5">
                <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-amber-300 font-black text-slate-950">{index + 1}</span>
